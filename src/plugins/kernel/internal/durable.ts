...
export const PipelineFailure = z.object({ runId: z.string().min(1), step: z.string().min(1), scope: z.string(), reason: z.enum(["retries", "abandoned"]) }).strict();
...
export const ABANDON = Symbol("fail a durable run whose job the schedule gave up");
...
export type DurableSteps = {
    ...
    [FAIL]: (runId: string, step: string, reason: "retries" | "abandoned") => Promise<void>;
