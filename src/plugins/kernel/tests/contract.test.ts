...
describe("names", () =>
{
    ...
    {
        expect(() => participant("Auth")).toThrow(KernelFault);
        expect(() => participant("auth_plugin")).toThrow(/lowercase letters, digits and hyphens/);
    });

    test("names the character that broke the name", () =>
    {
        expect(() => participant("auth plugin")).toThrow(/position 5/);
    });
...
describe("dependencies", () =>
{
    ...
    test("sets up a plugin after the ones it depends on", async () =>
    {
        ...
        const kernel = createKernel({
            plugins: [
                participant("billing", { dependsOn: ["auth"], setup: () => void began.push("billing") }),
                participant("auth", { setup: () => void began.push("auth") }),
            ],
        });
    ...
    {
        const kernel = createKernel({
            plugins: [participant("auth", { services: () => ({ token: "x" }) }), participant("billing")],
        });
...
describe("config", () =>
{
    ...
    test("hands a plugin its own parsed config", async () =>
    {
        let seen: unknown;
        const kernel = createKernel({
            plugins: [participant("billing", { config: z.object({ pageSize: z.number() }), setup: (ctx) => void (seen = ctx.config) })],
