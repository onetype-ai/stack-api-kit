...
function createRouteReading(reads?: readonly string[]): Plugin
{
...
describe("what a route may read", () =>
{
    ...
    {
        const kernel = createKernel({ plugins: [createRouteReading(["accept-language"])] });
    ...
    {
        const kernel = createKernel({ plugins: [createRouteReading()] });
    ...
    {
        const kernel = createKernel({ plugins: [createRouteReading(["accept-language", "x-missing"])] });
...
describe("what a route may never read", () =>
{
    ...
    for (const header of credentials)
    {
        ...
        {
            const kernel = createKernel({ plugins: [createRouteReading([header])] });
