...
describe("answers", () =>
{
    ...
    {
        answering({ status: 402, body: '{"error":"card declined"}' });
...
describe("cancelling", () =>
{
    test("tells a caller's abort apart from a timeout", async () =>
    {
        ...
        const calling = dial()({ method: "GET", url: "https://api.example.test/x", signal: stopper.signal });

        stopper.abort();

        await expect(calling).rejects.toMatchObject({ code: "ABORTED" });
    });
