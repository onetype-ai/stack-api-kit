...
- The database opens and migrates **before** any plugin runs, in dependency
  order. `database` takes a path or a `Store`; left out, nothing opens and a
  plugin declaring tables is refused by name.
- `outbox: true` keeps events until heard; `schedule: true` runs what is
  due here, `"enqueue"` only stores it (`procedures/work.md`).
- Who is calling comes from the plugin declaring `identifies`, so this file
  names no plugin; `identify` here overrides it, given the started kernel.
- The kernel starts **before** the server is built, so a refused contract
  stops everything with no route mounted.
- `limits: false` allows every request, and says so loudly.
