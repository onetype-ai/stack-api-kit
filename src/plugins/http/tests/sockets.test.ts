...
function sending(channel: string, reach: ChannelMessage["reach"], from?: Identity, scope?: string, fromConnection?: string): ChannelMessage
{
    return { channel, message: {}, reach, requires: [], scope, from, fromConnection };
}
...
describe("what a push reaches", () =>
{
    ...
    {
        const hub = sockets(await started(), "shopId");
        const heard: string[] = [];

        hub.subscribe(undefined, (text) => heard.push(text)).listen("chat.open");
        hub.push(sending("chat.open", "everyone"));
    ...
    test("a scope hears only its own", async () =>
    {
        ...
        hub.subscribe(who("a", "acme"), (text) => acme.push(text)).listen("chat.said");
        hub.subscribe(who("b", "beta"), (text) => beta.push(text)).listen("chat.said");

        hub.push(sending("chat.said", "scope", who("a", "acme"), "acme"));
    ...
    test("a viewer hears every socket of their own, and nobody else's", async () =>
    {
        ...
        hub.subscribe(who("a", "acme"), (text) => mine.push(text)).listen("chat.mine");
        hub.subscribe(who("a", "acme"), (text) => mine.push(text)).listen("chat.mine");
        hub.subscribe(who("b", "acme"), (text) => theirs.push(text)).listen("chat.mine");

        hub.push(sending("chat.mine", "viewer", who("a", "acme")));
    ...
    test("a channel that requires something is refused to whoever lacks it", async () =>
    {
        ...
        expect(hub.subscribe(who("a", "acme"), () => undefined).listen("chat.held")).toBe(false);
        expect(hub.subscribe(who("b", "acme", ["chat.read"]), () => undefined).listen("chat.held")).toBe(true);
    });
    ...
    test("and nobody hears a channel they never listened to", async () =>
    {
        ...
        hub.subscribe(undefined, (text) => heard.push(text));
        hub.push(sending("chat.open", "everyone"));
    ...
    test("nor one they left", async () =>
    {
        ...
        const subscription = hub.subscribe(undefined, (text) => heard.push(text));

        subscription.listen("chat.open");
        subscription.close();
    ...
    test("nor a scope channel while carrying no scope at all", async () =>
    {
        ...
        expect(hub.subscribe(who("a"), () => undefined).listen("chat.said")).toBe(false);
    });
});
